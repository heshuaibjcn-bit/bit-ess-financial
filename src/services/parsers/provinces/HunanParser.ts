...
export class HunanParser extends BaseProvinceParser {
  readonly provinceCode = 'HN';
  readonly provinceName = '湖南省';
  ...
  canParse(text: string, metadata: PDFMetadata): boolean {
    ...
    return textToCheck.includes('湖南') || 
           textToCheck.includes('湘发改价格') ||
           textToCheck.includes('湘价电') ||
           textToCheck.includes('长沙');
  }
  ...
  async parse(text: string, metadata: PDFMetadata): Promise<ParsedTariffData> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 提取价格
      const price = this.extractHunanPrice(line, lines, i);
    ...
    if (tariffItems.length === 0) {
      throw new Error('未能从 PDF 中提取到湖南省的电价数据');
    }
    ...
    return {
      ...
      publisher: metadata.publisher || '湖南省发展和改革委员会',
      tariffItems,
   ...
   * 提取湖南电价
   */
  private extractHunanPrice(line: string, lines: string[], index: number): number | null {
  ...
  private extractPolicyNumber(text: string): string | undefined {
    const match = text.match(/湘发改价格〔\d{4}〕\d+号/);
    if (match) return match[0];
    
    const match2 = text.match(/湘价电〔\d{4}〕\d+号/);
...
// 自动注册
parserRegistry.register(new HunanParser());
