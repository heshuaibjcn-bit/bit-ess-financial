...
export class HubeiParser extends BaseProvinceParser {
  readonly provinceCode = 'HB';
  readonly provinceName = '湖北省';
  ...
  canParse(text: string, metadata: PDFMetadata): boolean {
    ...
    return textToCheck.includes('湖北') || 
           textToCheck.includes('鄂发改价格') ||
           textToCheck.includes('鄂价电') ||
           textToCheck.includes('武汉');
  }
  ...
  async parse(text: string, metadata: PDFMetadata): Promise<ParsedTariffData> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 提取价格
      const price = this.extractHubeiPrice(line, lines, i);
    ...
    if (tariffItems.length === 0) {
      throw new Error('未能从 PDF 中提取到湖北省的电价数据');
    }
    ...
    return {
      ...
      publisher: metadata.publisher || '湖北省发展和改革委员会',
      tariffItems,
   ...
   * 提取湖北电价
   */
  private extractHubeiPrice(line: string, lines: string[], index: number): number | null {
  ...
  private extractPolicyNumber(text: string): string | undefined {
    const match = text.match(/鄂发改价格〔\d{4}〕\d+号/);
    if (match) return match[0];
    
    const match2 = text.match(/鄂价电〔\d{4}〕\d+号/);
...
// 自动注册
parserRegistry.register(new HubeiParser());
