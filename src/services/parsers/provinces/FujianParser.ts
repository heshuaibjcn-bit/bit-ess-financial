...
export class FujianParser extends BaseProvinceParser {
  readonly provinceCode = 'FJ';
  readonly provinceName = '福建省';
  ...
  canParse(text: string, metadata: PDFMetadata): boolean {
    ...
    return textToCheck.includes('福建') || 
           textToCheck.includes('闽发改价格') ||
           textToCheck.includes('闽价电') ||
           textToCheck.includes('福州');
  }
  ...
  async parse(text: string, metadata: PDFMetadata): Promise<ParsedTariffData> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 提取价格
      const price = this.extractFujianPrice(line, lines, i);
    ...
    if (tariffItems.length === 0) {
      throw new Error('未能从 PDF 中提取到福建省的电价数据');
    }
    ...
    return {
      ...
      publisher: metadata.publisher || '福建省发展和改革委员会',
      tariffItems,
   ...
   * 提取福建电价
   */
  private extractFujianPrice(line: string, lines: string[], index: number): number | null {
  ...
  private extractPolicyNumber(text: string): string | undefined {
    const match = text.match(/闽发改价格〔\d{4}〕\d+号/);
    if (match) return match[0];
    
    const match2 = text.match(/闽价电〔\d{4}〕\d+号/);
...
// 自动注册
parserRegistry.register(new FujianParser());
